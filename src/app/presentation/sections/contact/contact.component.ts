  ...
  template: `
    <section id="contact" class="relative mx-auto max-w-5xl px-6 py-24">
      ...
      >
        ...
        <p class="font-mono text-sm font-medium text-accent-400">{{ loc.t('contact.eyebrow') }}</p>
        <h2 class="mt-3 font-display text-4xl font-bold tracking-tight text-white sm:text-5xl">
          {{ loc.t('contact.titleLead') }} <span class="text-gradient">{{ loc.t('contact.titleAccent') }}</span>{{ loc.t('contact.titleTrail') }}
        </h2>
        <p class="mx-auto mt-5 max-w-xl text-lg text-slate-300">{{ loc.t('contact.subtitle') }}</p>
        ...
        <div class="mt-8 flex justify-center gap-3">
          ...
          <a [href]="profile().website" target="_blank" rel="noopener" aria-label="Website"
            class="flex h-11 w-11 items-center justify-center rounded-xl border border-white/10 bg-white/5 text-slate-300 transition-colors hover:bg-white/10 hover:text-white">
      ...
      <footer class="mt-16 border-t border-white/10 pt-8 text-center text-sm text-slate-500">
        <p>© {{ year }} {{ profile().name }}. {{ loc.t('contact.footer') }} · Clean Architecture.</p>
